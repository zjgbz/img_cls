...
if __name__ == "__main__":
    ...
    for epoch in range(cfg.max_epoch):
        ...
        # 模型保存
        if best_F1_macro < F1_macro_valid or epoch == cfg.max_epoch - 1:
            best_epoch = epoch if best_F1_macro < F1_macro_valid else best_epoch
            best_F1_macro = F1_macro_valid if best_F1_macro < F1_macro_valid else best_F1_macro
    ...
    logger.info("{} done, best F1(macro): {} in :{}".format(
        datetime.strftime(datetime.now(), '%m-%d_%H-%M'), best_F1_macro, best_epoch))